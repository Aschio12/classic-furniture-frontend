...
export default function HubManagerPage() {
  ...
  useEffect(() => {
    async function fetchHubOrders() {
      try {
        // In a real app, this endpoint would filter by the manager's assigned hub
        // For now, we fetch 'hub/pending' or all orders and filter client-side
        // pending endpoint is likely what we want if built previously
        const { data } = await api.get("/orders/hub/pending");
        
        // Filter specifically for 'Arrived at Hub' as requested
        // If the backend returns mixed statuses, we filter here.
        // Assuming data is an array of orders.
        const arrivedOrders = data.filter((o: any) => o.status === "Arrived at Hub");
  ...
  const handleVerifyHandover = async () => {
    ...
    try {
      ...
      // Assuming endpoint: /api/orders/:id/handover or similar
      // If not exists, we'll mock it for UI demo
      await api.put(`/orders/${selectedOrder._id}/status`, { status: "Completed" });
      ...
      alert("Failed to verify handover. Check console.");
    }
  };
  ...
  const hubName = user?.name ? `${user.name}'s Center` : "Adama Central Hub"; // Fallback or dynamic
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {orders.length === 0 ? (
          <div className="col-span-full py-20 text-center text-primary/40">
            ...
            <p>No orders currently at the hub.</p>
          </div>
        ) : (
          orders.map((order) => {
